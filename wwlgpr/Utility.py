...
def cal_node_weights(graph, atoms, cutoff, inner_cutoff, inner_weight, outer_weight, all_adsorbate=True):
    ...
    for i, bonding_index in enumerate(total_bonding_index):
        shortest_path.append(
            list(map(lambda x: len(x), graph.get_shortest_paths(bonding_index)))
        )
...
def ConcAttributes(node_attributes):
    conc_attributes = np.asarray(
        np.concatenate(tuple(i for i in node_attributes), axis=0), dtype=float
    )
...
def CumsumAttributes(conc_attributes, indexlist):
    cumsum_attributes = np.array(
        np.array_split(conc_attributes, indexlist[:-1]), dtype=object
    )
...
def HstackAttribute(attributes1, attributes2):
    ...
    indexlist = np.cumsum(
        list(map(lambda node_attribute: len(node_attribute), attributes1))
    )
...
def OrigianAttribute(node_attributes):
    ...
    indexlist = np.cumsum(
        list(map(lambda node_attribute: len(node_attribute), node_attributes))
    )
