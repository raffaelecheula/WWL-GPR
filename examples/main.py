...
def SCV5(
    ml_dict, opt_dimensions, default_para, fix_hypers,
):
    """task1 and task2: 5-fold cross validation for in-domain prediction stratified by adsorbate
     ...
     """
    global output_name
    db_graphs, db_atoms, node_attributes, list_ads_energies, file_names = load_db(
        ml_dict, "train"
    )
    test_RMSEs = []
    f_times = 1
    ...
    for train_index, vali_index in skf.split(
        list_ads_energies, ClassifySpecies(file_names)
    ):
...
def SCV5_FHP(ml_dict, fix_hypers):
...
def Extrapolation(ml_dict, fix_hypers):
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Physic-inspired Wasserstein Weisfeiler-Lehman Graph Gaussian Process Regression"
    )
    parser.add_argument(
        "--task",
        type=str,
    ...
    if args.task == "CV5":
        ...
        gpr_reg = Real(
            name="regularization of gpr", low=1e-3, high=1e0, prior="uniform"
        )
        gpr_len = Real(name="lengthscale of gpr", low=1, high=100, prior="uniform")
        # gpr_sigma    = 1
        edge_s_s = Real(
            name="edge weight of surface-surface", low=0, high=1, prior="uniform"
        )
        edge_s_a = Real(
            name="edge weight of surface-adsorbate", low=0, high=1, prior="uniform"
        )
        edge_a_a = Real(
            name="edge weight of adsorbate-adsorbate", low=0, high=1, prior="uniform"
        )
    ...
    if args.task.startswith("Extrapolation"):
        ...
        if args.task == "Extrapolation_t1":
            with open(
                base_path + "/database/complexads_extrapolation/" + "input_CuCo_Pt.yml"
            ) as f:
                ml_dict = yaml.safe_load(f)

        if args.task == "Extrapolation_t2":
            with open(
                base_path + "/database/complexads_extrapolation/" + "input_PdRh_Ru.yml"
            ) as f:
                ml_dict = yaml.safe_load(f)
        ...
        Extrapolation(ml_dict, fix_hypers)
