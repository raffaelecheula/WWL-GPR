...
def WorkFunction(outputfile, E_fermi):
    ...
    for i, line in enumerate(lines):
        if line.startswith(" CRYSTAL"):
            lines = lines[i:]
            xsf_start = i
        if line.startswith("DATAGRID_3D_UNKNOWN") or line.startswith(
...
def d_band_moment(atoms, outputfile):
    ...
    for atom_id in range(len(atoms)):
        ...
        dbc = simps(summed_pdos * dos_energies, dos_energies) / simps(
            summed_pdos, dos_energies
        )
        d_band_center_list.append(dbc)
        d_band_mom2 = simps(
            summed_pdos * np.power(dos_energies - dbc, 2), dos_energies
        ) / simps(summed_pdos, dos_energies)
        dbw = np.sqrt(d_band_mom2)
        d_band_width_list.append(dbw)
        d_band_mom3 = simps(
            summed_pdos * np.power(dos_energies - dbc, 3), dos_energies
        ) / simps(summed_pdos, dos_energies)
        dbs = d_band_mom3 / np.power(dbw, 3)
        d_band_skewness_list.append(dbs)
        d_band_mom4 = simps(
            summed_pdos * np.power(dos_energies - dbc, 4), dos_energies
        ) / simps(summed_pdos, dos_energies)
